...
export default function Navbar({ lang, theme, onLangChange, onThemeToggle, systemName }: any) {
    ...
    <nav className={`p-4 border-b flex flex-col md:flex-row justify-between items-center sticky top-0 z-[100] backdrop-blur-md ${isLight ? 'bg-white border-black' : 'bg-black/95 border-cyan-900/50'}`}>
      <div className="flex items-center gap-2 mb-3 md:mb-0">
        ...
        <div className="flex flex-col">
          <h1 className={`font-black italic text-xl select-none leading-none ${isLight ? 'text-black' : 'text-cyan-400'}`}>
            {systemName}
          </h1>
      ...
      <div className="flex items-center gap-4">
        <div className="flex gap-1 bg-black/10 p-1 rounded-xl">
          {langs.map(l => (
            <button key={l.id} onClick={() => onLangChange(l.id)} className={`px-3 py-1 rounded-lg text-[10px] font-black transition-all ${lang === l.id ? 'bg-cyan-600 text-white shadow-glow' : 'opacity-40 hover:opacity-100'}`}>{l.l}</button>
          ))}
        </div>
        <button onClick={onThemeToggle} className="p-2 border rounded-full border-gray-500 transition-transform active:scale-90">
